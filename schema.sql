CREATE TABLE jobs (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  job_number TEXT NOT NULL UNIQUE,
  job_title TEXT NOT NULL,
  press_id INTEGER,
  print_method TEXT,
  color_count INTEGER DEFAULT 0,
  status TEXT DEFAULT 'active',
  target_units INTEGER DEFAULT 0,
  curr_units INTEGER DEFAULT 0,
  print_units INTEGER DEFAULT 0,
  prev_units INTEGER DEFAULT 0,
  target_waste_percent REAL DEFAULT 3,
  actual_waste INTEGER DEFAULT 0,
  actual_stops INTEGER DEFAULT 0,
  setup_start_at TEXT,
  first_pull_at TEXT,
  prod_start_at TEXT,
  completed_at TEXT,
  setup_target_min INTEGER DEFAULT 0,
  admin_pull_count INTEGER DEFAULT 0,
  admin_waste_m INTEGER DEFAULT 0,
  admin_out INTEGER DEFAULT 0,
  admin_done INTEGER DEFAULT 0,
  admin_start_at TEXT,
  turningbar_used INTEGER DEFAULT 0,
  turningbar_config TEXT,
  created_at TEXT DEFAULT (datetime('now')),
  updated_at TEXT DEFAULT (datetime('now'))
);

CREATE TABLE benchmarks (
  id              INTEGER PRIMARY KEY AUTOINCREMENT,
  job_id          INTEGER,
  created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
  date            TEXT,
  time            TEXT,
  machine         TEXT,       -- Press name
  process_type    TEXT,       -- 'gravure' | 'flexo'
  color_name      TEXT,
  color_hex       TEXT,
  de              REAL,
  ds              REAL,
  delta_c         REAL,
  delta_h         REAL,
  delta_l         REAL,
  viscosity       REAL,
  print_speed     REAL,
  drying_temp     REAL,
  measurement_seq INTEGER,
  status_en       TEXT,
  status_tr       TEXT,
  benchmark_en    TEXT,
  benchmark_tr    TEXT,
  diagnosis_en    TEXT,
  diagnosis_tr    TEXT,
  sctv_5 REAL, sctv_10 REAL, sctv_25 REAL, sctv_50 REAL, sctv_75 REAL,
  edited_at TEXT,
  edited_from TEXT
);

CREATE TABLE presses (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  machine TEXT,
  max_colors INTEGER DEFAULT 8,
  status TEXT DEFAULT 'idle',
  sort_order INTEGER DEFAULT 0,
  max_units INTEGER DEFAULT 8,
  turning_bar_options TEXT DEFAULT '[]',
  turning_bar_extra_min INTEGER DEFAULT 20,
  splice_waste_m INTEGER DEFAULT 250,
  target_speed_m_min INTEGER DEFAULT 300,
  max_waste_percent INTEGER DEFAULT 3
);

CREATE TABLE system_settings (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  pull_time_min INTEGER,
  pull_waste_m INTEGER,
  splice_waste_m INTEGER,
  target_speed_m_min INTEGER,
  max_waste_percent INTEGER
);

CREATE TABLE admin_pulls (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  job_id INTEGER NOT NULL,
  pull_number INTEGER NOT NULL,
  waste_m INTEGER DEFAULT 0,
  de_cyan REAL,
  de_magenta REAL,
  de_yellow REAL,
  de_black REAL,
  de_spot1 REAL,
  de_spot2 REAL,
  de_spot3 REAL,
  de_spot4 REAL,
  de_spot5 REAL,
  de_spot6 REAL,
  created_at TEXT DEFAULT (datetime('now'))
);
